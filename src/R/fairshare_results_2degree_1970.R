...
write_parquet(full_resp_country_df %>%
                mutate(E_gvt_full = E_gvt_pos + E_gvt_neg), 'results/ISO_fairshare_resp_emissions_1970_2100_2deg.parquet')
...
write_parquet(gap_iso_resp, 'results/ISO_fairshare_resp_gap_2022_2023_2deg.parquet')
...
write_parquet(gap_iso_resp_2030, 'results/ISO_fairshare_resp_gap_2022_2030_2deg.parquet')
write_csv(gap_iso_resp_2030, 'results/ISO_fairshare_resp_gap_2022_2030_2deg.csv')
...
write_parquet(full_capacity_country_df %>%
                mutate(E_country_full = E_country_pos + E_country_neg), 'results/ISO_fairshare_capc_emissions_1970_2100_2deg.parquet')
...
write_parquet(gap_capacity_country, 'results/ISO_fairshare_capacity_country_gap_2022_2023_2deg.parquet')
...
write_parquet(gap_capacity_country_2030, 'results/ISO_fairshare_capacity_country_gap_2022_2030_2deg.parquet')
write_csv(gap_capacity_country_2030, 'results/ISO_fairshare_capacity_country_gap_2022_2030_2deg.csv')
...
# full_resp_capc_country_df_alt <- full_capacity_country_df %>%
#   rename_with(~paste0(., "_capc"), -c(GID_0, year))%>%
#   left_join(full_resp_country_df %>% rename_with(~paste0(., "_resp"), -c(GID_0, year)), by = c("GID_0", "year"), suffix = c("_capc", "_resp"))%>%
#   mutate(E_country_pos = (E_country_pos_capc + E_gvt_pos_resp)/2)%>%
#   mutate(E_country_neg = (E_country_neg_capc + E_gvt_neg_resp)/2)
...
write_parquet(gap_capacity, 'results/ADM1_fairshare_capacity_region_gap_2022_2023_2deg.parquet')

gap_capacity_2030 <- full_capacity_region_df %>% filter(year %in% c('2022', '2030'))%>%
  arrange(GID_1,year)%>%
  group_by(GID_1)%>%
  mutate(gap_E_gvt = GHG_e - lag(GHG_e))

write_parquet(gap_capacity_2030, 'results/ADM1_fairshare_capacity_region_gap_2022_2030_2deg.parquet')
