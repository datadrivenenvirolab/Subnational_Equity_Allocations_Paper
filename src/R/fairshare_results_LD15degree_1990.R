...
IMP_C1_Emissions_og <- read_excel("data/LD15_scenario.xlsx", sheet = "data")%>%
  filter(grepl('Emissions', Variable))%>%
  pivot_longer(cols = 6:16, names_to = "year", values_to = "emissions")%>%
  filter(year >= 2020) %>%
  filter(emissions!=0) 
...
# Negative Emissions
# #input =  MtCO2/yr
negative_emiss <- read_excel("data/LD15_scenario.xlsx", sheet = "data")%>%
  filter(grepl('Emissions', Variable))%>%
  pivot_longer(cols = 6:16, names_to = "year", values_to = "emissions")%>%
  filter(year >= 2020) %>%
  filter(emissions>=0) %>%
...
write_parquet(emiss_2020_2100, 'results/GBL_emissions_2020_2100_LD15deg.parquet')
...
write_parquet(capacity_region_full_df, 'results/ADM1_fairshare_capacity_region_emissions_1970_2100_wide_LD15deg.parquet')
...
write_parquet(gap_capacity, 'results/ADM1_fairshare_capacity_region_gap_2022_2023_LD15deg.parquet')
...
write_parquet(gap_capacity_2030, 'results/ADM1_fairshare_capacity_region_gap_2022_2030_LD15deg.parquet')
